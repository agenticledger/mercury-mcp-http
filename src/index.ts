#!/usr/bin/env node
/**
 * Mercury MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Mercury secrets. It is a *client* of the Connections Broker
 * (https://connectionsbroker.agenticledger.ai), which vaults each user's Mercury
 ...
 *      `Authorization: Bearer <mercury-api-key>`, use it directly.
 *   2. Broker-first (default): derive the caller's `principal`, sign a short-lived
 *      JWT, ask the broker POST /token for the Mercury key, construct the client.
...
const SLUG = 'mercury';
const NAME = 'Mercury MCP Server';
const VERSION = '2.0.0';

/** Construct a MercuryClient from a broker-resolved (or raw passthrough) credential. */
function makeClient(credential: string): MercuryClient {
  return new MercuryClient(credential);
}
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: VERSION,
    description:
      'Mercury Banking — accounts, transactions, recipients, payments, invoicing, and treasury operations through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Mercury API key as Bearer.',
      },
    },
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves the Mercury key via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <mercury-api-key>',
    ],
  });
});
...
type ClientResolution =
  | { kind: 'client'; client: MercuryClient }
...
async function connectMessage(principal: string): Promise<string> {
  ...
  if ('error' in started) {
    return `Mercury isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      ...
      message:
        brokerProviderKind === 'static'
          ? 'Mercury is not connected for this caller yet. Connect it once via the link below (paste your Mercury API key into your platform’s broker connect flow), then run the tool again — it will work.'
          : 'Mercury is not connected for this caller yet. Open the connect link below once, then run the tool again — it will work.',
  ...
  if (raw) {
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { Authorization: 'Bearer <your-mercury-api-key>' },
      });
      return;
    }
